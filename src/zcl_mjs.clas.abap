CLASS zcl_mjs DEFINITION PUBLIC.
  ...
  PRIVATE SECTION.
    CONSTANTS:
      c_bc_push_num      TYPE i VALUE 1,
      c_bc_push_str      TYPE i VALUE 2,
      c_bc_push_bool     TYPE i VALUE 3,
      c_bc_push_undef    TYPE i VALUE 4,
      c_bc_pop           TYPE i VALUE 5,
      c_bc_dup           TYPE i VALUE 6,
      c_bc_load_slot     TYPE i VALUE 10,
      c_bc_load_name     TYPE i VALUE 11,
      c_bc_store_slot    TYPE i VALUE 12,
      c_bc_store_name    TYPE i VALUE 13,
      c_bc_def_slot      TYPE i VALUE 14,
      c_bc_def_name      TYPE i VALUE 15,
      c_bc_binop         TYPE i VALUE 20,
      c_bc_unary         TYPE i VALUE 21,
      c_bc_jump          TYPE i VALUE 30,
      c_bc_jump_false    TYPE i VALUE 31,
      c_bc_jump_true     TYPE i VALUE 32,
      c_bc_jump_not_null TYPE i VALUE 33,
      c_bc_eval_node     TYPE i VALUE 40,
      c_bc_return        TYPE i VALUE 41,
      c_bc_get_prop      TYPE i VALUE 50,
      c_bc_get_idx       TYPE i VALUE 51,
      c_bc_set_prop      TYPE i VALUE 52,
      c_bc_call_name     TYPE i VALUE 60,
      c_bc_call_method   TYPE i VALUE 61,
      c_bc_enter_scope   TYPE i VALUE 70,
      c_bc_exit_scope    TYPE i VALUE 71.

    " One-entry compiled-module cache: avoids lexing/parsing/compiling when the
    " same source is evaluated repeatedly in one internal session.
    CLASS-DATA gv_bc_cached_source TYPE string.
    CLASS-DATA gt_bc_cached_stmts  TYPE zif_mjs=>tt_nodes.
    CLASS-DATA gr_bc_cached_code   TYPE REF TO zif_mjs=>tt_bc_code.
    CLASS-DATA gv_bc_cache_valid   TYPE abap_bool.

    CLASS-METHODS bc_compile_nodes
      IMPORTING it_nodes             TYPE zif_mjs=>tt_nodes
                iv_use_slots         TYPE abap_bool DEFAULT abap_false
      RETURNING VALUE(rt_code)       TYPE zif_mjs=>tt_bc_code.
    CLASS-METHODS bc_compile_stmt
      IMPORTING ir_node              TYPE REF TO data
                iv_use_slots         TYPE abap_bool
      CHANGING  ct_code              TYPE zif_mjs=>tt_bc_code.
    CLASS-METHODS bc_compile_expr
      IMPORTING ir_node              TYPE REF TO data
                iv_use_slots         TYPE abap_bool
      CHANGING  ct_code              TYPE zif_mjs=>tt_bc_code.
    CLASS-METHODS bc_has_loop_control
      IMPORTING ir_node              TYPE REF TO data
      RETURNING VALUE(rv_found)      TYPE abap_bool.
    CLASS-METHODS bc_execute
      IMPORTING it_code              TYPE zif_mjs=>tt_bc_code
                io_env               TYPE REF TO zcl_mjs_env
      RETURNING VALUE(rs_val)        TYPE zif_mjs=>ty_value
      RAISING zcx_mjs_runtime.
...
CLASS zcl_mjs IMPLEMENTATION.
  ...
  METHOD eval.
    DATA lt_stmts TYPE zif_mjs=>tt_nodes.
    DATA lr_top_code TYPE REF TO zif_mjs=>tt_bc_code.

    IF gv_bc_cache_valid = abap_true AND gv_bc_cached_source = iv_source
        AND gr_bc_cached_code IS BOUND.
      lt_stmts = gt_bc_cached_stmts.
      lr_top_code = gr_bc_cached_code.
    ELSE.
      ...
      DATA(lt_top_code) = bc_compile_nodes( it_nodes = lt_stmts iv_use_slots = abap_false ).
      CREATE DATA gr_bc_cached_code.
      gr_bc_cached_code->* = lt_top_code.
      gt_bc_cached_stmts = lt_stmts.
      gv_bc_cached_source = iv_source.
      gv_bc_cache_valid = abap_true.
      lr_top_code = gr_bc_cached_code.
    ENDIF.
    ...
    bc_execute( it_code = lr_top_code->* io_env = lo_env ).
  ...
  METHOD call_function.
    ...
    " Lazily lower the annotated function body to bytecode, then execute it.
    IF <fn>-bc_code IS NOT BOUND.
      DATA(lt_fn_code) = bc_compile_nodes( it_nodes = <fn>-body iv_use_slots = abap_true ).
      CREATE DATA <fn>-bc_code.
      <fn>-bc_code->* = lt_fn_code.
    ENDIF.
    rs_val = bc_execute( it_code = <fn>-bc_code->* io_env = lo_call_env ).
  ...
  METHOD bc_compile_nodes.
    LOOP AT it_nodes INTO DATA(lr_node).
      bc_compile_stmt(
        EXPORTING ir_node      = lr_node
                  iv_use_slots = iv_use_slots
        CHANGING  ct_code      = rt_code ).
    ENDLOOP.
  ENDMETHOD.

  METHOD bc_compile_stmt.
    IF ir_node IS NOT BOUND.
      RETURN.
    ENDIF.
    FIELD-SYMBOLS <n> TYPE zif_mjs=>ty_node.
    ...
    CASE <n>-kind.
      WHEN zif_mjs=>c_node_var.
        IF <n>-right IS BOUND.
          bc_compile_expr( EXPORTING ir_node = <n>-right iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        ELSE.
          APPEND VALUE #( opcode = c_bc_push_undef ) TO ct_code.
        ENDIF.
        IF iv_use_slots = abap_true AND <n>-slot_ok = abap_true.
          APPEND VALUE #( opcode = c_bc_def_slot arg = <n>-slot ) TO ct_code.
        ELSE.
          APPEND VALUE #( opcode = c_bc_def_name text = <n>-str ) TO ct_code.
        ENDIF.
        APPEND VALUE #( opcode = c_bc_pop ) TO ct_code.

      WHEN zif_mjs=>c_node_if.
        bc_compile_expr( EXPORTING ir_node = <n>-cond iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        DATA(lv_if_false) = lines( ct_code ) + 1.
        APPEND VALUE #( opcode = c_bc_jump_false ) TO ct_code.
        LOOP AT <n>-body INTO DATA(lr_if_body).
          bc_compile_stmt( EXPORTING ir_node = lr_if_body iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        ENDLOOP.
        IF lines( <n>-els ) > 0.
          DATA(lv_if_end) = lines( ct_code ) + 1.
          APPEND VALUE #( opcode = c_bc_jump ) TO ct_code.
          ct_code[ lv_if_false ]-target = lines( ct_code ) + 1.
          LOOP AT <n>-els INTO DATA(lr_else_body).
            bc_compile_stmt( EXPORTING ir_node = lr_else_body iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
          ENDLOOP.
          ct_code[ lv_if_end ]-target = lines( ct_code ) + 1.
        ELSE.
          ct_code[ lv_if_false ]-target = lines( ct_code ) + 1.
        ENDIF.

      WHEN zif_mjs=>c_node_while.
        DATA(lv_while_control) = abap_false.
        LOOP AT <n>-body INTO DATA(lr_while_check).
          IF bc_has_loop_control( lr_while_check ) = abap_true.
            lv_while_control = abap_true.
            EXIT.
          ENDIF.
        ENDLOOP.
        IF lv_while_control = abap_true.
          APPEND VALUE #( opcode = c_bc_eval_node node = ir_node ) TO ct_code.
          APPEND VALUE #( opcode = c_bc_pop ) TO ct_code.
          RETURN.
        ENDIF.
        DATA(lv_while_start) = lines( ct_code ) + 1.
        bc_compile_expr( EXPORTING ir_node = <n>-cond iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        DATA(lv_while_exit) = lines( ct_code ) + 1.
        APPEND VALUE #( opcode = c_bc_jump_false ) TO ct_code.
        LOOP AT <n>-body INTO DATA(lr_while_body).
          bc_compile_stmt( EXPORTING ir_node = lr_while_body iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        ENDLOOP.
        APPEND VALUE #( opcode = c_bc_jump target = lv_while_start ) TO ct_code.
        ct_code[ lv_while_exit ]-target = lines( ct_code ) + 1.

      WHEN zif_mjs=>c_node_for.
        " The loop runs in a child lexical environment. Its instructions use
        " name lookup so loop locals stay in that child while outer function
        " slots are still reached through the parent chain.
        DATA(lv_for_control) = abap_false.
        LOOP AT <n>-body INTO DATA(lr_for_check).
          IF bc_has_loop_control( lr_for_check ) = abap_true.
            lv_for_control = abap_true.
            EXIT.
          ENDIF.
        ENDLOOP.
        IF lv_for_control = abap_true.
          APPEND VALUE #( opcode = c_bc_eval_node node = ir_node ) TO ct_code.
          APPEND VALUE #( opcode = c_bc_pop ) TO ct_code.
          RETURN.
        ENDIF.
        APPEND VALUE #( opcode = c_bc_enter_scope ) TO ct_code.
        IF <n>-init IS BOUND.
          bc_compile_stmt( EXPORTING ir_node = <n>-init iv_use_slots = abap_false CHANGING ct_code = ct_code ).
        ENDIF.
        DATA(lv_for_start) = lines( ct_code ) + 1.
        DATA lv_for_exit TYPE i.
        IF <n>-cond IS BOUND.
          bc_compile_expr( EXPORTING ir_node = <n>-cond iv_use_slots = abap_false CHANGING ct_code = ct_code ).
          lv_for_exit = lines( ct_code ) + 1.
          APPEND VALUE #( opcode = c_bc_jump_false ) TO ct_code.
        ENDIF.
        LOOP AT <n>-body INTO DATA(lr_for_body).
          bc_compile_stmt( EXPORTING ir_node = lr_for_body iv_use_slots = abap_false CHANGING ct_code = ct_code ).
        ENDLOOP.
        IF <n>-update IS BOUND.
          bc_compile_expr( EXPORTING ir_node = <n>-update iv_use_slots = abap_false CHANGING ct_code = ct_code ).
          APPEND VALUE #( opcode = c_bc_pop ) TO ct_code.
        ENDIF.
        APPEND VALUE #( opcode = c_bc_jump target = lv_for_start ) TO ct_code.
        IF lv_for_exit > 0.
          ct_code[ lv_for_exit ]-target = lines( ct_code ) + 1.
        ENDIF.
        APPEND VALUE #( opcode = c_bc_exit_scope ) TO ct_code.

      WHEN zif_mjs=>c_node_block.
        LOOP AT <n>-body INTO DATA(lr_block_body).
          bc_compile_stmt( EXPORTING ir_node = lr_block_body iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        ENDLOOP.

      WHEN zif_mjs=>c_node_return.
        IF <n>-left IS BOUND.
          bc_compile_expr( EXPORTING ir_node = <n>-left iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        ELSE.
          APPEND VALUE #( opcode = c_bc_push_undef ) TO ct_code.
        ENDIF.
        APPEND VALUE #( opcode = c_bc_return ) TO ct_code.

      WHEN OTHERS.
        bc_compile_expr( EXPORTING ir_node = ir_node iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        APPEND VALUE #( opcode = c_bc_pop ) TO ct_code.
    ENDCASE.
  ENDMETHOD.

  METHOD bc_compile_expr.
    IF ir_node IS NOT BOUND.
      APPEND VALUE #( opcode = c_bc_push_undef ) TO ct_code.
      RETURN.
    ENDIF.
    ...
    CASE <n>-kind.
      WHEN zif_mjs=>c_node_number.
        APPEND VALUE #( opcode = c_bc_push_num num = <n>-num ) TO ct_code.
      WHEN zif_mjs=>c_node_string.
        APPEND VALUE #( opcode = c_bc_push_str text = <n>-str ) TO ct_code.
      WHEN zif_mjs=>c_node_bool.
        APPEND VALUE #( opcode = c_bc_push_bool arg = CONV i( <n>-num ) ) TO ct_code.

      WHEN zif_mjs=>c_node_ident.
        IF iv_use_slots = abap_true AND <n>-slot_ok = abap_true.
          APPEND VALUE #( opcode = c_bc_load_slot arg = <n>-slot ) TO ct_code.
        ELSE.
          APPEND VALUE #( opcode = c_bc_load_name text = <n>-str ) TO ct_code.
        ENDIF.

      WHEN zif_mjs=>c_node_binop.
        IF <n>-op = `&&` OR <n>-op = `||` OR <n>-op = `??`.
          bc_compile_expr( EXPORTING ir_node = <n>-left iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
          APPEND VALUE #( opcode = c_bc_dup ) TO ct_code.
          DATA(lv_short_jump) = lines( ct_code ) + 1.
          IF <n>-op = `&&`.
            APPEND VALUE #( opcode = c_bc_jump_false ) TO ct_code.
          ELSEIF <n>-op = `||`.
            APPEND VALUE #( opcode = c_bc_jump_true ) TO ct_code.
          ELSE.
            APPEND VALUE #( opcode = c_bc_jump_not_null ) TO ct_code.
          ENDIF.
          APPEND VALUE #( opcode = c_bc_pop ) TO ct_code.
          bc_compile_expr( EXPORTING ir_node = <n>-right iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
          ct_code[ lv_short_jump ]-target = lines( ct_code ) + 1.
        ELSE.
          bc_compile_expr( EXPORTING ir_node = <n>-left iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
          bc_compile_expr( EXPORTING ir_node = <n>-right iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
          APPEND VALUE #( opcode = c_bc_binop text = <n>-op ) TO ct_code.
        ENDIF.

      WHEN zif_mjs=>c_node_unaryop.
        IF <n>-op = `-` OR <n>-op = `!`.
          bc_compile_expr( EXPORTING ir_node = <n>-left iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
          APPEND VALUE #( opcode = c_bc_unary text = <n>-op ) TO ct_code.
        ELSEIF <n>-op = `void`.
          bc_compile_expr( EXPORTING ir_node = <n>-left iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
          APPEND VALUE #( opcode = c_bc_pop ) TO ct_code.
          APPEND VALUE #( opcode = c_bc_push_undef ) TO ct_code.
      ...
      WHEN zif_mjs=>c_node_assign OR zif_mjs=>c_node_assign_add.
        ...
        bc_compile_expr( EXPORTING ir_node = <n>-right iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        IF <n>-kind = zif_mjs=>c_node_assign_add.
          APPEND VALUE #( opcode = c_bc_binop text = `+` ) TO ct_code.
        ENDIF.
        IF iv_use_slots = abap_true AND <n>-slot_ok = abap_true.
          APPEND VALUE #( opcode = c_bc_store_slot arg = <n>-slot ) TO ct_code.
        ELSE.
          APPEND VALUE #( opcode = c_bc_store_name text = <n>-str ) TO ct_code.
        ENDIF.

      WHEN zif_mjs=>c_node_inc OR zif_mjs=>c_node_dec
          OR zif_mjs=>c_node_post_inc OR zif_mjs=>c_node_post_dec.
        FIELD-SYMBOLS <inc> TYPE zif_mjs=>ty_node.
        ASSIGN <n>-left->* TO <inc>.
        IF sy-subrc <> 0 OR <inc>-kind <> zif_mjs=>c_node_ident.
          APPEND VALUE #( opcode = c_bc_eval_node node = ir_node ) TO ct_code.
          RETURN.
        ENDIF.
        bc_compile_expr( EXPORTING ir_node = <n>-left iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        IF <n>-kind = zif_mjs=>c_node_post_inc OR <n>-kind = zif_mjs=>c_node_post_dec.
          APPEND VALUE #( opcode = c_bc_dup ) TO ct_code.
        ENDIF.
        APPEND VALUE #( opcode = c_bc_push_num num = 1 ) TO ct_code.
        IF <n>-kind = zif_mjs=>c_node_inc OR <n>-kind = zif_mjs=>c_node_post_inc.
          APPEND VALUE #( opcode = c_bc_binop text = `+` ) TO ct_code.
        ELSE.
          APPEND VALUE #( opcode = c_bc_binop text = `-` ) TO ct_code.
        ENDIF.
        IF iv_use_slots = abap_true AND <inc>-slot_ok = abap_true.
          APPEND VALUE #( opcode = c_bc_store_slot arg = <inc>-slot ) TO ct_code.
        ELSE.
          APPEND VALUE #( opcode = c_bc_store_name text = <inc>-str ) TO ct_code.
        ENDIF.
        IF <n>-kind = zif_mjs=>c_node_post_inc OR <n>-kind = zif_mjs=>c_node_post_dec.
          APPEND VALUE #( opcode = c_bc_pop ) TO ct_code.
        ENDIF.
      ...
      WHEN zif_mjs=>c_node_member_access.
        DATA(lv_member_fallback) = boolc( <n>-op = `?.` ).
        FIELD-SYMBOLS <member_obj> TYPE zif_mjs=>ty_node.
        IF <n>-object IS BOUND.
          ASSIGN <n>-object->* TO <member_obj>.
          IF sy-subrc = 0 AND <member_obj>-kind = zif_mjs=>c_node_ident
              AND <member_obj>-str = `Object`.
            lv_member_fallback = abap_true.
          ENDIF.
        ENDIF.
        IF lv_member_fallback = abap_true.
          APPEND VALUE #( opcode = c_bc_eval_node node = ir_node ) TO ct_code.
          RETURN.
        ENDIF.
        bc_compile_expr( EXPORTING ir_node = <n>-object iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        IF <n>-prop_expr IS BOUND.
          bc_compile_expr( EXPORTING ir_node = <n>-prop_expr iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
          APPEND VALUE #( opcode = c_bc_get_idx ) TO ct_code.
        ELSE.
          APPEND VALUE #( opcode = c_bc_get_prop text = <n>-property ) TO ct_code.
        ENDIF.
      ...
      WHEN zif_mjs=>c_node_member_assign.
        ...
        bc_compile_expr( EXPORTING ir_node = <n>-object iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        bc_compile_expr( EXPORTING ir_node = <n>-right iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        APPEND VALUE #( opcode = c_bc_set_prop text = <n>-property node = <n>-object ) TO ct_code.

      WHEN zif_mjs=>c_node_call.
        DATA(lv_call_fallback) = boolc( <n>-left IS BOUND OR <n>-str = `super` ).
        LOOP AT <n>-args INTO DATA(lr_call_check).
          FIELD-SYMBOLS <call_arg> TYPE zif_mjs=>ty_node.
          ASSIGN lr_call_check->* TO <call_arg>.
          IF sy-subrc = 0 AND <call_arg>-op = `SPREAD`.
            lv_call_fallback = abap_true.
            EXIT.
          ENDIF.
        ENDLOOP.
        IF lv_call_fallback = abap_true.
          APPEND VALUE #( opcode = c_bc_eval_node node = ir_node ) TO ct_code.
          RETURN.
        ENDIF.
        LOOP AT <n>-args INTO DATA(lr_call_arg).
          bc_compile_expr( EXPORTING ir_node = lr_call_arg iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        ENDLOOP.
        APPEND VALUE #( opcode = c_bc_call_name arg = lines( <n>-args ) text = <n>-str node = ir_node ) TO ct_code.

      WHEN zif_mjs=>c_node_method_call.
        DATA(lv_method_fallback) = boolc( <n>-op = `?.` ).
        FIELD-SYMBOLS <method_obj> TYPE zif_mjs=>ty_node.
        IF <n>-object IS BOUND.
          ASSIGN <n>-object->* TO <method_obj>.
          IF sy-subrc = 0 AND <method_obj>-kind = zif_mjs=>c_node_ident
              AND ( <method_obj>-str = `Date` OR <method_obj>-str = `Object` OR <method_obj>-str = `super` ).
            lv_method_fallback = abap_true.
          ENDIF.
        ENDIF.
        LOOP AT <n>-args INTO DATA(lr_method_check).
          FIELD-SYMBOLS <method_arg> TYPE zif_mjs=>ty_node.
          ASSIGN lr_method_check->* TO <method_arg>.
          IF sy-subrc = 0 AND <method_arg>-op = `SPREAD`.
            lv_method_fallback = abap_true.
            EXIT.
          ENDIF.
        ENDLOOP.
        IF lv_method_fallback = abap_true.
          APPEND VALUE #( opcode = c_bc_eval_node node = ir_node ) TO ct_code.
          RETURN.
        ENDIF.
        bc_compile_expr( EXPORTING ir_node = <n>-object iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        LOOP AT <n>-args INTO DATA(lr_method_arg).
          bc_compile_expr( EXPORTING ir_node = lr_method_arg iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        ENDLOOP.
        APPEND VALUE #( opcode = c_bc_call_method arg = lines( <n>-args ) text = <n>-property node = <n>-object ) TO ct_code.

      WHEN zif_mjs=>c_node_ternary.
        bc_compile_expr( EXPORTING ir_node = <n>-cond iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        DATA(lv_ternary_false) = lines( ct_code ) + 1.
        APPEND VALUE #( opcode = c_bc_jump_false ) TO ct_code.
        bc_compile_expr( EXPORTING ir_node = <n>-left iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        DATA(lv_ternary_end) = lines( ct_code ) + 1.
        APPEND VALUE #( opcode = c_bc_jump ) TO ct_code.
        ct_code[ lv_ternary_false ]-target = lines( ct_code ) + 1.
        bc_compile_expr( EXPORTING ir_node = <n>-right iv_use_slots = iv_use_slots CHANGING ct_code = ct_code ).
        ct_code[ lv_ternary_end ]-target = lines( ct_code ) + 1.
  ...
  METHOD bc_has_loop_control.
    IF ir_node IS NOT BOUND.
      RETURN.
    ENDIF.
    FIELD-SYMBOLS <n> TYPE zif_mjs=>ty_node.
    ASSIGN ir_node->* TO <n>.
    IF <n>-kind = zif_mjs=>c_node_break OR <n>-kind = zif_mjs=>c_node_continue.
      rv_found = abap_true.
      RETURN.
    ENDIF.
    IF <n>-kind = zif_mjs=>c_node_func_decl.
      RETURN.
    ENDIF.
    IF bc_has_loop_control( <n>-left ) = abap_true
        OR bc_has_loop_control( <n>-right ) = abap_true
        OR bc_has_loop_control( <n>-cond ) = abap_true
        OR bc_has_loop_control( <n>-init ) = abap_true
        OR bc_has_loop_control( <n>-update ) = abap_true.
      rv_found = abap_true.
      RETURN.
    ENDIF.
    LOOP AT <n>-body INTO DATA(lr_body).
      IF bc_has_loop_control( lr_body ) = abap_true.
        rv_found = abap_true.
        RETURN.
      ENDIF.
    ENDLOOP.
    LOOP AT <n>-els INTO DATA(lr_else).
      IF bc_has_loop_control( lr_else ) = abap_true.
        rv_found = abap_true.
        RETURN.
      ENDIF.
    ENDLOOP.
  ...
  METHOD bc_execute.
    ...
    DATA ls_ins TYPE zif_mjs=>ty_bc_instr.
    DATA ls_a TYPE zif_mjs=>ty_value.
    DATA ls_b TYPE zif_mjs=>ty_value.
    DATA ls_result TYPE zif_mjs=>ty_value.
    DATA lv_found TYPE abap_bool.
    DATA lo_env TYPE REF TO zcl_mjs_env.
    DATA lo_child TYPE REF TO zcl_mjs_env.
    DATA lt_env_stack TYPE STANDARD TABLE OF REF TO zcl_mjs_env WITH EMPTY KEY.
    DATA lt_args TYPE zif_mjs=>tt_value_slots.
    DATA lv_base TYPE i.
    DATA lv_arg_index TYPE i.
    DATA lv_delete_count TYPE i.
    DATA lv_prop TYPE string.
    DATA ls_fn TYPE zif_mjs=>ty_value.
    DATA lr_box TYPE REF TO data.
    FIELD-SYMBOLS <set_obj_node> TYPE zif_mjs=>ty_node.

    lo_env = io_env.
    WHILE lv_ip <= lines( it_code ).
      READ TABLE it_code INDEX lv_ip INTO ls_ins.
      lv_ip = lv_ip + 1.

      CASE ls_ins-opcode.
        WHEN c_bc_push_num.
          CLEAR ls_result.
          ls_result-type = zif_mjs=>c_type_number.
          ls_result-num = ls_ins-num.
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.

        WHEN c_bc_push_str.
          CLEAR ls_result.
          ls_result-type = zif_mjs=>c_type_string.
          ls_result-str = ls_ins-text.
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.

        WHEN c_bc_push_bool.
          CLEAR ls_result.
          ls_result-type = zif_mjs=>c_type_bool.
          ls_result-num = ls_ins-arg.
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.

        WHEN c_bc_push_undef.
          CLEAR ls_result.
          ls_result-type = zif_mjs=>c_type_undefined.
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.

        WHEN c_bc_pop.
          IF lv_sp > 0.
            DELETE lt_stack INDEX lv_sp.
            lv_sp = lv_sp - 1.
          ENDIF.

        WHEN c_bc_dup.
          READ TABLE lt_stack INDEX lv_sp INTO ls_result.
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.

        WHEN c_bc_load_slot.
          CLEAR ls_result.
          READ TABLE io_env->slots INDEX ls_ins-arg INTO ls_result.
          IF sy-subrc <> 0.
            ls_result-type = zif_mjs=>c_type_undefined.
          ENDIF.
          APPEND ls_result TO lt_stack.
        ...
        WHEN c_bc_load_name.
          lo_env->resolve(
            EXPORTING iv_name = ls_ins-text
            IMPORTING ev_found = lv_found es_val = ls_result ).
          IF lv_found = abap_false.
            raise_ref_error( ls_ins-text ).
          ENDIF.
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.

        WHEN c_bc_store_slot.
          READ TABLE lt_stack INDEX lv_sp INTO ls_result.
          io_env->set_slot( iv_slot = ls_ins-arg is_val = ls_result ).

        WHEN c_bc_store_name.
          READ TABLE lt_stack INDEX lv_sp INTO ls_result.
          lo_env->set( iv_name = ls_ins-text is_val = ls_result ).
        ...
        WHEN c_bc_def_name.
          READ TABLE lt_stack INDEX lv_sp INTO ls_result.
          lo_env->define( iv_name = ls_ins-text is_val = ls_result ).

        WHEN c_bc_binop.
          READ TABLE lt_stack INDEX lv_sp INTO ls_b.
          DELETE lt_stack INDEX lv_sp.
          lv_sp = lv_sp - 1.
          READ TABLE lt_stack INDEX lv_sp INTO ls_a.
          DELETE lt_stack INDEX lv_sp.
          lv_sp = lv_sp - 1.
          ls_result = eval_bin_op(
            iv_op = ls_ins-text is_left = ls_a is_right = ls_b io_env = lo_env ).
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.
        ...
        WHEN c_bc_unary.
          ...
          IF ls_ins-text = `-`.
            ls_result-type = zif_mjs=>c_type_number.
            ls_result-num = 0 - zcl_mjs_val=>to_number( ls_a ).
          ELSE.
            ls_result-type = zif_mjs=>c_type_bool.
            IF zcl_mjs_val=>is_true( ls_a ) = abap_false.
              ls_result-num = 1.
            ENDIF.
          ENDIF.
          APPEND ls_result TO lt_stack.
        ...
        WHEN c_bc_jump_false OR c_bc_jump_true OR c_bc_jump_not_null.
          READ TABLE lt_stack INDEX lv_sp INTO ls_a.
          DELETE lt_stack INDEX lv_sp.
          lv_sp = lv_sp - 1.
          IF ( ls_ins-opcode = c_bc_jump_false AND zcl_mjs_val=>is_true( ls_a ) = abap_false )
              OR ( ls_ins-opcode = c_bc_jump_true AND zcl_mjs_val=>is_true( ls_a ) = abap_true )
              OR ( ls_ins-opcode = c_bc_jump_not_null
                   AND ls_a-type <> zif_mjs=>c_type_undefined
                   AND ls_a-type <> zif_mjs=>c_type_null ).
            lv_ip = ls_ins-target.
          ENDIF.

        WHEN c_bc_eval_node.
          ls_result = eval_node( ir_node = ls_ins-node io_env = lo_env ).
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.
        ...
        WHEN c_bc_get_prop.
          ...
          DELETE lt_stack INDEX lv_sp.
          lv_sp = lv_sp - 1.
          ls_result = eval_property_access(
            is_obj = ls_a iv_prop = ls_ins-text io_env = lo_env ).
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.
        ...
        WHEN c_bc_get_idx.
          ...
          DELETE lt_stack INDEX lv_sp.
          lv_sp = lv_sp - 1.
          lv_prop = zcl_mjs_val=>to_string( ls_b ).
          ls_result = eval_property_access(
            is_obj = ls_a iv_prop = lv_prop io_env = lo_env ).
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.
        ...
        WHEN c_bc_set_prop.
          ...
          DELETE lt_stack INDEX lv_sp.
          lv_sp = lv_sp - 1.
          lr_box = zcl_mjs_val=>box_value( ls_b ).
          IF ls_a-type = zif_mjs=>c_type_object AND ls_a-obj IS BOUND.
            ls_a-obj->set( iv_key = ls_ins-text ir_val = lr_box ).
          ELSEIF ls_a-type = zif_mjs=>c_type_function.
            IF ls_a-obj IS NOT BOUND.
              CREATE OBJECT ls_a-obj.
            ENDIF.
            ls_a-obj->set( iv_key = ls_ins-text ir_val = lr_box ).
            IF ls_ins-node IS BOUND.
              ASSIGN ls_ins-node->* TO <set_obj_node>.
              IF sy-subrc = 0 AND <set_obj_node>-kind = zif_mjs=>c_node_ident.
                lo_env->set( iv_name = <set_obj_node>-str is_val = ls_a ).
              ENDIF.
            ENDIF.
          ENDIF.
          APPEND ls_b TO lt_stack.
          lv_sp = lv_sp + 1.

        WHEN c_bc_call_name.
          CLEAR lt_args.
          lv_base = lv_sp - ls_ins-arg + 1.
          DO ls_ins-arg TIMES.
            lv_arg_index = lv_base + sy-index - 1.
            READ TABLE lt_stack INDEX lv_arg_index INTO ls_a.
            APPEND ls_a TO lt_args.
          ENDDO.
          lv_delete_count = ls_ins-arg.
          DO lv_delete_count TIMES.
            DELETE lt_stack INDEX lv_sp.
            lv_sp = lv_sp - 1.
          ENDDO.
          CLEAR ls_result.
          IF ls_ins-text = `Boolean`.
            ls_result = zcl_mjs_builtins=>boolean_from_args( lt_args ).
          ELSEIF ls_ins-text = `RegExp`.
            ls_result = zcl_mjs_builtins=>regexp_from_args( lt_args ).
          ELSEIF ls_ins-text = `JSON.stringify`.
            ls_result = zcl_mjs_builtins=>json_stringify( lt_args ).
          ELSEIF ls_ins-text = `console.log`.
            ls_result = zcl_mjs_builtins=>console_log( it_args = lt_args io_env = lo_env ).
          ELSE.
            lo_env->resolve(
              EXPORTING iv_name = ls_ins-text
              IMPORTING ev_found = lv_found es_val = ls_fn ).
            IF lv_found = abap_true AND ls_fn-type = zif_mjs=>c_type_function AND ls_fn-fn IS BOUND.
              ls_result = call_function( ir_fn = ls_fn-fn it_args = lt_args io_env = lo_env ).
            ELSE.
              ls_result = zcl_mjs_val=>undefined_val( ).
            ENDIF.
          ENDIF.
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.

        WHEN c_bc_call_method.
          CLEAR lt_args.
          lv_base = lv_sp - ls_ins-arg.
          READ TABLE lt_stack INDEX lv_base INTO ls_a.
          DO ls_ins-arg TIMES.
            lv_arg_index = lv_base + sy-index.
            READ TABLE lt_stack INDEX lv_arg_index INTO ls_b.
            APPEND ls_b TO lt_args.
          ENDDO.
          lv_delete_count = ls_ins-arg + 1.
          DO lv_delete_count TIMES.
            DELETE lt_stack INDEX lv_sp.
            lv_sp = lv_sp - 1.
          ENDDO.
          ls_result = eval_method_call(
            is_obj = ls_a iv_method = ls_ins-text it_args = lt_args
            io_env = lo_env ir_obj_node = ls_ins-node ).
          APPEND ls_result TO lt_stack.
          lv_sp = lv_sp + 1.

        WHEN c_bc_enter_scope.
          APPEND lo_env TO lt_env_stack.
          CREATE OBJECT lo_child EXPORTING io_parent = lo_env.
          lo_env = lo_child.

        WHEN c_bc_exit_scope.
          IF lo_env->parent IS BOUND.
            lo_env = lo_env->parent.
          ENDIF.
          DELETE lt_env_stack INDEX lines( lt_env_stack ).
      ENDCASE.

      IF lo_env->returning = abap_true.
        io_env->returning = abap_true.
        io_env->ret_val = lo_env->ret_val.
        rs_val = lo_env->ret_val.
        RETURN.
      ENDIF.
    ENDWHILE.
